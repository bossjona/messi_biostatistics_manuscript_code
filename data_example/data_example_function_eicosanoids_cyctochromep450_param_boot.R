...
analysis.fn.eicosanoids <- function(phthalate_visit, metabolite.nm){
  ...
  d2 <- read.csv("protect.all.exposures.csv", stringsAsFactors = FALSE)
  ...
  other.df <- d2 %>% filter(visit == phthalate_visit) %>% dplyr::select(studyid, FINALGA_BEST, isage, mage_cat, edu_cat, preBMI,
                                                                        preBMI_cat, sg, metabolite.nm)
  ...
  print("Hard Constraint - Unpenalized Done")
...
#For now just grab visit
other.df <- d2 %>% filter(visit == phthalate_visit) %>% dplyr::select(studyid, FINALGA_BEST, isage, mage_cat, edu_cat, preBMI,
                                                                      preBMI_cat, sg, all_of(metabolite.nm))

#Merge datasets
mediator.model.df <- merge(other.df, eico.df, by = "studyid")
mediator.model.df <- mediator.model.df %>% filter(!is.na(FINALGA_BEST), !is.na(isage), !is.na(mage_cat), !is.na(edu_cat),
                                                  !is.na(preBMI), !is.na(preBMI_cat), !is.na(sg))
